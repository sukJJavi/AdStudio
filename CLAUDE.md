...
- **`adstudio_formats`** — por proyecto: `iab_format`, `nombre_soporte`, `status` (`pending`/`producing`/`ready`/`incident`), `incidencias[]`, `copy`, `peso_max_kb`, `is_master` (uno solo por proyecto), `soportes[]` (medios/plataformas que necesitan ese tamaño), `source_psd_id` (PSD propio de este formato), `master_base_psd_id` (override manual del master-base asignado por ratio — ver "Múltiples PSDs independientes")
...
- **`adstudio_masters`** — un master real por cada PSD subido (`source_psd_id` no nulo, ver "Múltiples PSDs independientes"), con `is_primary`, `format_id` (formato del plan asociado explícitamente, si lo hay) y `html` (HTML5 completo, editable por chat); también reutilizada para las adaptaciones producidas (`format_id` no nulo, `source_psd_id` nulo, `status` `draft`/`ready`)
...
## Múltiples PSDs independientes — un master por PSD

Un proyecto puede tener varios PSDs subidos. Cada PSD genera su propio master HTML5 independiente (no solo el marcado `is_master`), y cada formato del plan se asigna al master-base (PSD) cuyo ratio de aspecto es más cercano — no a un único "master global".

1. **Extracción** (`trigger/analyze-psd.ts`) — cada capa extraída guarda `source_psd_id` con el id del asset del PSD (`adstudio_assets.layer_type = 'psd'`) del que proviene. El propio asset PSD guarda además sus dimensiones en `metadata.psdWidth`/`metadata.psdHeight` — necesarias por-PSD porque `adstudio_projects.psd_width`/`psd_height` es un único campo a nivel proyecto que el último PSD procesado sobrescribe.
2. **Brief** (`components/project/brief-form.tsx`) — con más de un PSD subido se muestra "Material por formato": un select por PSD para asociarlo al formato del plan que produce (`adstudio_formats.source_psd_id`, vía `PATCH /api/brief/formats/[formatId]`). Con un único PSD, se asocia automáticamente al formato master. Además, cada fila de la tabla de soportes tiene un select "Master base" (`adstudio_formats.master_base_psd_id`) para forzar manualmente el master-base de ESE formato cuando la asignación automática por ratio no es la deseada — "Automático (más cercano)" por defecto.
...
4. **Master** (`trigger/render-master.ts`) — genera un HTML5 independiente para CADA PSD subido (no solo el `is_master` o el de mayor área), usando solo las capas de ese PSD (`source_psd_id`) y sus propias dimensiones (`psdWidth`/`psdHeight`). Sube a `{project_id}/masters/{source_psd_id}/...` y registra una fila en `adstudio_masters` con `source_psd_id` (identifica un master real; distinto de `format_id`, que solo aporta copy/clickTag si el PSD tiene un formato del plan asociado explícitamente). El master primario (`is_primary`) además alimenta `adstudio_projects.master_html` — el preview principal (`/api/preview/[projectId]`), el chat de cambios (`lib/master.ts:refineMasterHtml`) y el link de aprobación al cliente siguen operando sobre ESE campo de proyecto; los demás masters se previsualizan/editan vía `/api/preview/[projectId]/master/[psdId]` y el mismo `refineMasterHtml` parametrizado con `sourcePsdId`.
5. **Producción — asignación de formato a master-base** (`trigger/render-adaptations.ts:assignMasterToFormat`) — para cada formato del plan: se calcula el ratio de aspecto de cada master (PSD) y se asigna al más cercano; si la diferencia es < 5% se considera "match exacto" (se copia el master de ese PSD ajustando solo el px de sobra, sin FLUX ni Claude). Si no, se clasifica Nivel 1/Nivel 2 (`classifyFormat`) **frente a su master-base asignado**, no frente a un master global. Prioridad de asignación: `master_base_psd_id` (override manual) > `source_psd_id` (asociación histórica del formato) > automático por ratio. CRÍTICO: los assets de cada formato se descargan EXCLUSIVAMENTE del PSD asignado (`source_psd_id` del master-base) — nunca se mezclan capas de PSDs distintos para un mismo formato, ni en el HTML ni en el ZIP.

La aprobación del cliente (`/approve/[token]`) sigue operando sobre `adstudio_projects.master_html`/`master_run_id` a nivel de proyecto (el master primario) — con varios masters, la vista de Master (`app/project/[id]/master`) los lista todos, cada uno con su propio iframe y chat de cambios independiente, pero el flujo de aprobación de cliente aplica solo al primario.
